...
### Release Workflow
1. Tag pushed → `release.yml` runs
2. Builds 3 DMG variants (ARM64, x86_64, universal)
3. Signs universal DMG with EdDSA key from GitHub Secret
4. `generate-appcast.sh` creates `appcast.xml` with enclosure signature
5. Pushes to `docs/` branch → GitHub Pages serves it
6. `softprops/action-gh-release` creates GitHub release with DMGs

### Development Notes
- Sparkle requires `com.apple.security.cs.disable-library-validation` entitlement for ad-hoc signed development builds
- `startingUpdater: false` + manual `updater.start()` required to avoid init-time crash
