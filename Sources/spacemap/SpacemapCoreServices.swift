...
final class SpacemapCoreServices {
    ...
    init(
        ...
        self.yabaiService = yabaiService ?? Self.detectWindowManager()
    ...
    private static func detectWindowManager() -> YabaiService {
        let yabai = YabaiClientImpl()
        if yabai.isYabaiRunning(forceRefresh: true) { return yabai }
        let aerospace = AeroSpaceClient()
        if aerospace.isYabaiRunning(forceRefresh: true) { return aerospace }
        return yabai
    }
